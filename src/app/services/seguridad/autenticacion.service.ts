...
export class AutenticacionService {
  ...
  public check(ruta:string): Observable<Boolean>{
      let _json = JSON.stringify({ ruta: ruta });
      ...
      return this._http.post(app.apiurl+'/seguridad.php/check', _params, {headers: _headers})
          .map((response: Response) => {
              ...
              let data=this.extractData(response,false);
              if(data.success) return true;
              this._helper.notificationToast("No posee los privilegios necesarios para acceder al recurso solicitado. Contacto con nosotros para mayor información.","Acceso denegado","info");
              return false;
          }).catch(err=>this.handleError(err));
  }
  public login(usuario: string, clave: string): Observable<boolean> {
    let json = JSON.stringify({ usuario: usuario, clave: clave });
